...
import { MACHINE_SLUG } from "@/data/products";
...
export default function CartDrawer() {
  ...
  const whatsappUrl = `https://wa.me/${contact.whatsappNumber.replace(/\+/g, "")}?text=${encodeURIComponent(
    `Hi! I'd like to place an order:\n\n${items.map(item => `- ${item.productName} x${item.quantity}${item.isFreeWithSubscription ? ' (FREE)' : ''}`).join('\n')}\n\nTotal: ${formatPrice(totalPrice)}${isSubscription ? '\n\nI want to subscribe for monthly delivery.' : ''}`
  )}`;
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted mt-1 capitalize">
                        {item.productType === "cialde" ? "Coffee Cialde" : "Espresso Machine"}
                      </p>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <div className="flex-1">
                      ...
                      <span className="text-xs text-muted block mt-1">
                        Get a free espresso machine with your order
                      </span>
                    </div>
                  </label>
                </div>
        ...
        {items.length > 0 && (
          <div className="border-t border-[var(--border)] p-6 space-y-4">
            ...
            <p className="text-xs text-center text-muted">
              {isSubscription ? "Monthly subscription order" : "One-time purchase"}
            </p>
          </div>
        )}
      </div>
